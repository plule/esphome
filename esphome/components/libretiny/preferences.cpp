...
ESPPreferenceObject LibreTinyPreferences::make_preference(size_t length, uint32_t type, bool in_flash) {
...
bool LibreTinyPreferences::is_changed_(fdb_kvdb_t db, const void *to_save_ptr, const char *key_str) {
  const auto &to_save = *static_cast<const NVSData *>(to_save_ptr);
  struct fdb_kv kv;
